...
def parseGRID(lines, i):
    ...
    while 'TRNX' not in x:
        j = j + 1
        x = lines[j+1]
    (xind0, xind1) = (j + 3, j + 4 + gridPts[0])
    
    while 'TRNY' not in x:
        j = j + 1
        x = lines[j+1]
    (yind0, yind1) = (j + 3, j + 4 + gridPts[1])
    
    while 'TRNZ' not in x:
        j = j + 1
        x = lines[j+1]
    (zind0, zind1) = (j + 3, j + 4 + gridPts[2])
    ...
    #print(xlines, xind0, xind1)
    #print(ylines, yind0, yind1)
    #print(zlines, zind0, zind1)
    for x in xlines:
...
def parseSMVFile(smvFile):
    ...
    for i in range(0, len(linesSMV)):
        ...
        if 'SLCC' in linesSMV[i]:
            ...
            files['SLICES'][file]['SHORTNAME'] = linesSMV[i+3].strip()
            files['SLICES'][file]['UNITS'] = linesSMV[i+4].strip()
        if 'ROOM' in linesSMV[i]:
            room = linesSMV[i+1].strip().split() + linesSMV[i+2].strip().split()
            room = [float(x) for x in room]
            rooms.append(room)
    return grids, obsts, bndfs, surfs, files, rooms
